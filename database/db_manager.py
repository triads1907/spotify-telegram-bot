...
class DatabaseManager:
    ...
    async def sync_telegram_track(self, audio_data: dict) -> bool:
        """
        Синхронизировать трек из Telegram в базу данных.
        audio_data: {file_id, name, artist, message_id}
        """
        async with self.async_session() as session:
            # 1. Проверяем, есть ли уже этот файл в TelegramFile по file_id
            # (так как один и тот же трек может иметь разныеSpotify ID, но один file_id в канале)
            result = await session.execute(
                select(TelegramFile).where(TelegramFile.file_id == audio_data['file_id'])
            )
            existing_file = result.scalar_one_or_none()
            
            if existing_file:
                return False # Уже синхронизирован
            
            # 2. Создаем уникальный ID для трека (если нет Spotify ID)
            # Используем префикс tg_ + короткий хеш или часть file_id
            import hashlib
            safe_id = "tg_" + hashlib.md5(audio_data['file_id'].encode()).hexdigest()[:16]
            
            # 3. Проверяем, нет ли уже такого трека в таблице tracks
            result = await session.execute(select(Track).where(Track.id == safe_id))
            track = result.scalar_one_or_none()
            ...
            if not track:
                track = Track(
                    ...
                    spotify_url=f"https://t.me/c/{config.STORAGE_CHANNEL_ID.replace('-100', '')}/{audio_data['message_id']}",
                    image_url=None # В сканируемых аудио обложек обычно нет в Bot API объекте напрямую
                )
                session.add(track)
            
            # 4. Создаем запись в TelegramFile
            telegram_file = TelegramFile(
                track_id=safe_id,
                file_id=audio_data['file_id'],
                artist=audio_data['artist'],
                track_name=audio_data['name']
            )
            session.add(telegram_file)
